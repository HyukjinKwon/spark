...
class KafkaRedactionUtilSuite extends SparkFunSuite with KafkaDelegationTokenTest {
  ...
  test("redactJaasParam should redact secrets in all quoting styles and clientSecret") {
    ...
    cases.foreach { case (param, secret) =>
      ...
      assert(!redacted.contains(secret), s"secret leaked in redacted output: $redacted")
