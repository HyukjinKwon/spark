...
package object config extends Logging {
  ...
  private[spark] val AM_TRUST_PROXY_USER_COOKIE =
    ConfigBuilder("spark.yarn.am.trustProxyUserCookie")
      .doc("When true (default), the YARN AM UI filter trusts the 'proxy-user' cookie set by the " +
        "YARN RM web proxy to determine the user for the AM UI view/modify ACLs. That cookie is " +
        "not cryptographically verified, so a request from a proxy host can forge it. Set to " +
        "false to stop trusting the cookie: proxy requests are then treated as having no " +
        "user, which disables per-user AM UI ACLs through the proxy but removes the spoofing " +
        "vector. A complete fix requires the RM web proxy to sign the cookie.")
      .version("4.3.0")
      .booleanConf
      .createWithDefault(true)
