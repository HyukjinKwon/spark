...
class AmIpFilterSuite extends SparkFunSuite {
  ...
  test("proxy-user cookie is ignored when TRUST_PROXY_USER is false") {
    ...
    when(request.getCookies).thenReturn(
      Array(new Cookie(AmIpFilter.PROXY_USER_COOKIE_NAME, "attacker")))
    ...
    // Default (parameter absent) trusts the cookie: the request is wrapped with the forged user.
    val trusted = capturedRequest(None)
    assert(trusted.isInstanceOf[AmIpServletRequestWrapper])
    assert(trusted.asInstanceOf[HttpServletRequest].getRemoteUser === "attacker")

    // TRUST_PROXY_USER=false: the cookie is ignored and the request passes through unwrapped.
    val untrusted = capturedRequest(Some("false"))
    assert(!untrusted.isInstanceOf[AmIpServletRequestWrapper])
  }
