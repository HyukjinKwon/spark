...
object AttributeGroup {
  ...
  private[attribute] def fromMetadata(metadata: Metadata, name: String): AttributeGroup = {
    ...
    // `NUM_ATTRIBUTES` is read from column metadata that may originate from an untrusted data
    ...
    // above it: a valid-but-huge count (near Int.MaxValue) still drives an unbounded allocation.
