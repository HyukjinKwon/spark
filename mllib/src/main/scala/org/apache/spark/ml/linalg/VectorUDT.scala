      ...
      case row: InternalRow =>
        ...
        tpe match {
          case 0 =>
            ...
            // The declared size becomes the dense length on densification, so a crafted huge size
            // can drive an unbounded allocation. `MLMaxNumFeatures.get` is a cached read of a
            // static conf (no per-row allocation), and the check is a no-op unless the limit is
            // set. Only the sparse branch pays it; dense vectors are unaffected.
