...
object MLUtils extends Logging {
  ...
  private[spark] def computeNumFeatures(rdd: RDD[(Double, Array[Int], Array[Double])]): Int = {
    ...
    // The feature dimension is inferred from the largest index in an untrusted libsvm file, and
    // then used as the (dense) size of every parsed vector. A crafted file with a huge index can
    // force an unbounded allocation. When `spark.sql.ml.maxNumFeatures` is set to a positive
